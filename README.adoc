...
:sectnums:
:sectnumlevels: 3
:description: RGT Vault (Reasonably Good Token Vault) - Post-quantum secure identity vault for SSH keys, API tokens, PGP keys, and digital credentials. A parody of Pretty Good Privacy. Open source, formally verified.
:keywords: RGT Vault, Reasonably Good Token Vault, password manager, identity vault, SSH key storage, PGP key manager, API token security, post-quantum cryptography, Kyber, Dilithium, secure credential storage, open source security, PGP parody
...
image:https://img.shields.io/badge/License-AGPL--3.0-orange[License: AGPL-3.0]
image:https://img.shields.io/badge/Status-Alpha-yellow[Status: Alpha]

== What Is RGT Vault?

RGT Vault (Reasonably Good Token Vault) stores your digital identities securely. Named as a parody of Pretty Good Privacy (PGP), it goes beyond typical password managers to protect your most sensitive credentials. Built on the Svalinn container with the corre-terro image.
...
# Container deployment (uses Svalinn with corre-terro image)
podman pull ghcr.io/hyperpolymath/rgt-vault:latest
----
...
# Add an SSH key
rgt-vault add ssh --name "my-server-key" --path ~/.ssh/id_ed25519

# Add a GitHub token
rgt-vault add pat --name "github-token" --host github.com

# List your credentials (shows GUIDs only)
rgt-vault list

# Retrieve a credential
rgt-vault get a7f2c3d4-e5b6-4a8c-9d0e-f1a2b3c4d5e6
----
...
=== Container Architecture (Svalinn + corre-terro)

RGT Vault runs inside a Svalinn container using the corre-terro image:
...
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │     SVALINN CONTAINER (corre-terro image) - Delivery     │   │
│  │                                                          │   │
...
│                             │ API Socket Only                   │
│  ┌──────────────────────────┴──────────────────────────────┐   │
│  │     SVALINN CONTAINER (corre-terro image) - Data         │   │
...
RGT Vault is in *alpha* development. The core architecture is solid, but:
...
*RGT Vault* — Reasonably Good Token Vault. A parody of Pretty Good Privacy, for tokens.

*Built on Svalinn* — From Norse mythology, the shield that protects the world from the sun's heat.

_Your credentials deserve protection that lasts. And a sense of humour._
