...
# setup.sh — Universal setup script for reasonably-good-token-vault
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/reasonably-good-token-vault/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== reasonably-good-token-vault Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/reasonably-good-token-vault.git"
        info "Then: cd reasonably-good-token-vault && ./setup.sh"
        exit 1
    fi
